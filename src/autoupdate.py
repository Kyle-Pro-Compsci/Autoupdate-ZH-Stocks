...
class Autoupdater:
    ...
    def update_current_prices_group(self, wb: pyxl.Workbook, all_stocks, file_index):
        
        holdings_sheet = wb[self.config.sheet_name(file_index)]
        for row in holdings_sheet.iter_rows(min_row=self.config.starting_row(file_index)):
            price_cell = row[self.config.current_price_column(file_index) - 1]
            stock_name = row[self.config.stock_name_column(file_index) - 1].value
            ...
            if stock_name is None or stock_name in self.ENDCELLS:
    ...
    def update_current_prices_individual(self, wb: pyxl.Workbook, file_index):
        ...
        for row in holdings_sheet.iter_rows(min_row=self.config.starting_row(file_index)):
            name_cell = row[self.config.stock_name_column(file_index) - 1]
            ...
            row[self.config.current_price_column(file_index) - 1].value = price
            sleep(random.uniform(3,6))


    def update_current_prices(self, wb: pyxl.Workbook, all_stocks, file_index):
        if all_stocks is not None:
            try:
                self.update_current_prices_group(wb, all_stocks, file_index)
            except Exception as e:
                logging.error(e)
                logging.info("批量查询失败")
                self.update_current_prices_individual(wb, file_index)
        else:
            self.update_current_prices_individual(wb, file_index)


    def save_excel_workbook(self, wb: pyxl.Workbook, path):
    ...
    def run(self):
        setup_logger(self.config.verbose())
        ...
        all_stocks = None
        if self.config.should_update_prices():
        ...
        for index, file in enumerate(self.config.file_list()):
            xlsx_path = Path(self.config.xlsx_filename(index))
            ...
            if self.config.should_update_prices():
                self.update_current_prices(wb, all_stocks, index) #TODO: THIS - update to use 
        
            self.save_excel_workbook(wb, xlsx_path)
